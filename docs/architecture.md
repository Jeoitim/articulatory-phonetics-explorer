...
后缩舌根在上表面末端与下缘起始处共用 `rootTangent`：后缩越明显，切线越接近向下，使控制点代表后壁的一段而非被拉长的尖端。下缘随后独立回到下方附着处。该规则由位置连续混合，适用于预设、咽化变体和手动编辑。后舌预设的前舌高度另受形状回归检查保护，不能为降低二维面积而强压舌冠；参见[舌形校准与影像参考](laminal-shape-review.md)。
